#pragma once

#include <memory>
#include <string>
#include <atomic>
#include <vector>

#if defined(_WIN64) || defined(_WIN32)
#define NOMINMAX
#include <Windows.h>
#undef MOUSE_MOVED
#include "curses.h"
#else
#include <ncurses.h>
#include <linux/limits.h>
#define MAX_PATH PATH_MAX
#endif

#include "sqlite3.h"
#include "Card.h"
// #include "DB_API.h"

#if defined(__x86_64) || defined(_M_X64)
#define DATATYPE unsigned long long
#elif defined(__i386) || defined(_M_IX86)
#define DATATYPE unsigned int
#endif

// global variables
extern std::atomic<bool> g_paused;
extern std::atomic<bool> g_started;
extern std::atomic<float> g_progress;

namespace console
{
	// Represents a button with a label and associated action.
	class Button
	{
	public:
		Button(std::string label, int action) : m_label(label), m_action(action) {}
		std::string m_label;
		int m_action;
	};

	// Internal utility functions for the console application.

	// Prints a banner with version and URL information.
	void print_banner(const std::string &version, const std::string &url);

	// Checks if two strings are equal in a case-insensitive manner.
	inline bool case_insensitive_equals(const std::string &input, const std::string &comp);

	// Checks if the input string indicates a quit command.
	bool check_quit(const std::string &input);

	// Gets a SQLite database and returns it along with potential error messages.
	std::shared_ptr<sqlite3> get_db(std::string &db_path, std::string &err_msg);

	// Draws buttons on the console.
	void draw_buttons(const std::vector<console::Button> &buttons, size_t curr_btn_idx);

	// Displays a yes/no prompt on the console and returns the user's choice.
	int yes_no(const std::string &str);

	// Adds a card with user-inputted details.
	Card add_card();

	// Gets a formatted string representation of a card.
	inline std::string get_card_string(int max_length, const Card &card);

	// Draws a list of cards on the console.
	void draw_cards(const std::vector<Card> &cards_vec, int start_row, int visible_rows, int max_offset, int curr_row_idx, const std::vector<bool> &selected_rows);

	// Guides the user in choosing cards and returns the user's action.
	int choose_cards(std::shared_ptr<sqlite3> db, const std::string &db_path, std::vector<Card> &cards_vec, std::vector<bool> &cards_selection);

	// Guides the user in choosing the amount of data to generate and returns the user's action.
	int choose_amount(DATATYPE &amount, const std::string &db_path);

	// Gets a valid file path from the user.
	void get_path(std::string &path);

	// Guides the user in choosing a file and returns the user's action.
	int choose_file(std::string &exp_path);

	// Guides the user in generating data and returns the user's action.
	int generate(const std::string &exp_path, const std::vector<Card> &cards_vec, const std::vector<bool> &cards_selection, DATATYPE amount, std::shared_ptr<sqlite3> db);

	// Let the user to enable/disable CVV, name, and date.
	int choose_options();

	// Let the user to choose expiration date range
	int choose_date();
}