...
void Card::generate_card(std::ostringstream &oss) const
{
	...
	// Reserve memory for the string to avoid reallocations
	rnd_card.reserve(m_len);

	for (int i = rnd_card.size(); i < m_len - 1; i++)
	{
		rnd_card += '0' + dis(m_rng);
	}
...
bool Card::validate_prefix(const std::string &prefix, int len)
{
	...
	while (std::getline(ss, token, ','))
	{
		...
		{
			// the prefix is larger than the length	or if it's zero
...
/**
 * @brief Generates a 3-digit CVV and appends it to the provided string stream.
 *
 * This function generates a 3-digit CVV using random numbers between 0 and 9, appends
 * it to the given string stream (`std::ostringstream`), and separates the CVV with the
 * specified delimiter.
 *
 * @param delimiter The character used to separate the CVV from other content in the stream.
 * @param oss The output string stream where the generated CVV will be appended.
 *
 * @see std::uniform_int_distribution
 */
void Card::generate_cvv(const char delimiter, std::ostringstream &oss)
{
	std::uniform_int_distribution<int> dis(0, 9);
	oss << delimiter;
...
/**
 * @brief Generates a random date between the given start and end dates and appends it to the provided string stream.
 *
 * This function calculates a random date within the range specified by `start` and `end`
 * using a random number generator. The generated date is then appended to the given string
 * stream (`std::ostringstream`) with the specified delimiter.
 *
 * @param delimiter The character used to separate the date from other content in the stream.
 * @param start The starting date of the range.
 * @param end The ending date of the range.
 * @param oss The output string stream where the generated date will be appended.
 *
 * @see std::chrono::sys_days
 * @see std::uniform_int_distribution
 */
void Card::generate_date(const char delimiter, std::chrono::year_month_day start, std::chrono::year_month_day end, std::ostringstream &oss)
{
	// Convert to sys_days (number of days since 1970-01-01)
	std::chrono::sys_days start_days{start};
	std::chrono::sys_days end_days{end};

	// Calculate range in days
	auto days_range = (end_days - start_days).count();

	// Random number generator
	std::uniform_int_distribution<int> dis(0, days_range);

	// Random offset from start
	std::chrono::sys_days random_day = start_days + std::chrono::days{dis(m_rng)};
	std::chrono::year_month_day random_date{random_day};
