...
// include: C:\Users\AsusYI\AppData\Local\Temp\tmptzmsbmgq.js
  ...
  (() => {
    ...
    function loadPackage(metadata) {
      ...
      var PACKAGE_NAME = 'CC_Generator/CC_Generator_GUI.data';
      var REMOTE_PACKAGE_BASE = 'CC_Generator_GUI.data';
...
var REMOTE_PACKAGE_SIZE = metadata['remote_package_size'];
    ...
    function runWithFS(Module) {
      ...
      function processPackageData(arrayBuffer) {
        ...
        // Reuse the bytearray from the XHR as the source for file reads.
          ...
          }          Module['removeRunDependency']('datafile_CC_Generator/CC_Generator_GUI.data');

      };
      Module['addRunDependency']('datafile_CC_Generator/CC_Generator_GUI.data');
    ...
    loadPackage({"files": [{"filename": "/Cards.db", "start": 0, "end": 126976}], "remote_package_size": 126976});

  })();

// end include: C:\Users\AsusYI\AppData\Local\Temp\tmptzmsbmgq.js
// include: C:\Users\AsusYI\AppData\Local\Temp\tmpo004j03a.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if ((typeof ENVIRONMENT_IS_WASM_WORKER != 'undefined' && ENVIRONMENT_IS_WASM_WORKER) || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD) || (typeof ENVIRONMENT_IS_AUDIO_WORKLET != 'undefined' && ENVIRONMENT_IS_AUDIO_WORKLET)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: C:\Users\AsusYI\AppData\Local\Temp\tmpo004j03a.js
// include: C:\Users\AsusYI\AppData\Local\Temp\tmppumv0ze2.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: C:\Users\AsusYI\AppData\Local\Temp\tmppumv0ze2.js
...
function findWasmBinary() {
    return locateFile('CC_Generator_GUI.wasm');
}
...
var ASM_CONSTS = {
  348976: ($0, $1) => { HEAPU8[$0] = 0; let input = document.createElement('input'); window.selectedFilePath = null; input.type = 'file'; input.accept = UTF8ToString($1); input.onchange = e => { let file = e.target.files[0]; let reader = new FileReader(); reader.onload = function () { let data = new Uint8Array(reader.result); let filename = "/tmp/" + file.name; FS.writeFile(filename, data); console.log("File written to VFS:", filename); window.selectedFilePath = filename; HEAPU8[$0] = 1; }; reader.readAsArrayBuffer(file); }; input.click(); },  
 349502: ($0, $1, $2) => { const path = UTF8ToString($0); const downloadName = UTF8ToString($1); const flagPtr = $2; try { const data = FS.readFile(path); const blob = new Blob([data], { type: 'application/octet-stream' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = downloadName; a.setAttribute('download', downloadName); a.click(); URL.revokeObjectURL(url); console.log("Download triggered:", path); } catch (e) { console.error("Failed to download file:", e); } HEAPU8[flagPtr] = 0; },  
 350026: ($0) => { var url = UTF8ToString($0); window.open(url, '_blank'); }
};
function set_background_color(color) { document.documentElement.style.setProperty('background-color', UTF8ToString(color)); document.body.style.setProperty('background-color', UTF8ToString(color)); }
